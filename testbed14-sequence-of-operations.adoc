...
The CWL workflow steps perform runs of Docker containers while the EMS shall invoke the appropriate ADES to execute the processes. Instead of tailoring cwl-runner, Spacebel approach consist in rewriting the workflow steps to match the desired behaviour.

For each workflow step, the appropriate target ADES is computed depending on its inputs, and a command line step is created for starting a WPS commmand-line client and perform the execute request.
