...
. Alice sister's discover the new application and build a workflow including that application.
. Alice's sister's deploys the workflow.
. Bob discovers the new workflow and execute it.

The execution of workflow or an application on the EMS internally includes the following steps:

. Discovery of the EO Image products using the catalogs gateway.
. Deployment on ADES
. Execution on ADES
